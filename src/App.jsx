      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#EBE5E0] overflow-hidden print:shadow-none print:border-none">
          <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[800px] print:min-w-0">
                  <thead><tr className="bg-[#F9F7F5] text-[#936142] text-xs uppercase tracking-wider print:bg-transparent print:border-b print:border-[#414942]"><th className="p-4 font-semibold w-[40%]">Статья</th><th className="p-4 font-semibold w-28">План</th><th className="p-4 font-semibold w-28">Факт</th><th className="p-4 font-semibold w-28">Внесено</th><th className="p-4 font-semibold w-28">Остаток</th><th className="p-4 font-semibold w-48">Комментарий</th><th className="p-4 font-semibold w-10 print:hidden"></th></tr></thead>
                  ...
                  {expenses.map((item, idx) => (
                      ...
                      <td className="p-4 align-top"><MoneyInput value={item.plan} onChange={(val) => updateExpense(idx, 'plan', val)} className="w-full text-[#414942]" /></td>
                      <td className="p-4 align-top"><MoneyInput value={item.fact} onChange={(val) => updateExpense(idx, 'fact', val)} className="w-full text-[#414942]" /></td>
                      <td className="p-4 align-top"><MoneyInput value={item.paid} onChange={(val) => updateExpense(idx, 'paid', val)} className="w-full text-[#414942]" /></td>
                      <td className="p-4 align-top text-[#AC8A69]">{formatCurrency(item.fact - item.paid)}</td>
                      <td className="p-4 align-top"><textarea className="w-full bg-transparent outline-none text-xs text-[#AC8A69] placeholder-[#CCBBA9] resize-y min-h-[40px] leading-tight" placeholder="..." value={item.note || ''} onChange={(e) => updateExpense(idx, 'note', e.target.value)} /></td>
                      <td className="p-4 align-top print:hidden"><button onClick={() => removeExpense(idx)} className="text-red-300 hover:text-red-500 md:opacity-0 md:group-hover:opacity-100 transition-opacity"><Trash2 size={16} /></button></td>
                      </tr>
                  ))}
                  </tbody>
              </table>
...
export default function WeddingPlanner() {
  ...
  // 1. Authenticate with Firebase & Auto-Login
  useEffect(() => {
      ...
      <div className="min-h-screen bg-[#F9F7F5] font-[Montserrat] flex items-center justify-center p-6 print:hidden">
        <Card className="w-full max-w-2xl p-8 md:p-12 animate-slideUp">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#F9F7F5] p-4 rounded-xl flex items-center gap-3 border border border-[#AC8A69]/20">
