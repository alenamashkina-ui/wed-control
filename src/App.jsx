...
const INITIAL_FORM_STATE = { groomName: '', brideName: '', date: '', guestsCount: '', prepLocation: 'home', registrationType: 'official', venueName: '', clientPassword: '' };
...
// --- UI COMPONENTS ---
const Logo = ({ className }) => ( <img src={LOGO_URL} alt={APP_TITLE} className={`object-contain ${className}`} /> );

const Footer = () => (
    <footer className="mt-auto py-8 text-center text-[#CCBBA9] text-xs border-t border-[#EBE5E0] w-full print:hidden">
        <p className="mb-2">© {new Date().getFullYear()} ПараПланнер</p>
...
export default function WeddingPlanner() {
          ...
          const unsub = onSnapshot(query(collection(db, 'artifacts', appId, 'public', 'data', 'users'), where('role', '==', 'super_admin')), (snapshot) => {
              if (snapshot.empty) {
                  addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'users'), {
                      email: 'owner@wed.control', password: 'admin', name: 'Владелец Платформы', role: 'super_admin', agencyId: 'super_admin_id', secret: 'secret'
                  });
              }
              unsub(); 
          });
    ...
    try {
        ...
        if (foundUser) {
            const userData = { 
                ...
                agencyId: foundUser.agencyId || 'legacy_agency' 
            };
            setUser(userData);
  ...
  const handleCreateProject = async () => {
    ...
    } catch (e) { console.error(e); alert("Ошибка создания. Проверьте консоль."); } finally { setIsCreating(false); }
  };
