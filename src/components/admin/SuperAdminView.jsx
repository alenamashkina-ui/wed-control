...
import { 
  ...
  Briefcase, Users, Calendar, ListFilter
} from 'lucide-react';
...
export function SuperAdminView({ onLogout, currentUser }) {
  ...
  const processedAgencies = useMemo(() => {
      ...
      return admins.map(agency => {
          ...
          const employees = rawUsers.filter(u => 
              u.role === 'organizer' && (u.agencyId === targetId || u.agencyId === agency.id)
          );

          const projects = rawProjects.filter(p => 
              p.agencyId === targetId || p.agencyId === agency.id || p.organizerId === agency.id
          );
  ...
  const openCreateModal = () => { setEditingAgency(null); setFormData({ name: '', email: '', password: '', phone: '' }); setShowModal(true); };
  const openEditModal = (agency) => { setEditingAgency(agency); setFormData({ name: agency.name || '', email: agency.email || '', password: agency.password || '', phone: agency.phone || '' }); setShowModal(true); };
      ...
      <div className="max-w-6xl mx-auto w-full flex-1">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                    <span className="text-[#AC8A69] font-medium">Кабинет: {currentUser?.name || 'Super Admin'}</span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 border-b border-[#EBE5E0] pb-6">
            <div>
                ...
                <p className="text-[#CCBBA9]">Всего зарегистрировано: {processedAgencies.length}</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                ...
                <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-[#EBE5E0]">
                    <ListFilter size={16} className="text-[#AC8A69]" />
                    <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent text-sm text-[#414942] font-medium outline-none cursor-pointer hover:text-[#936142] transition-colors">
          ...
          {processedAgencies.length === 0 && !loading && (
              <div className="text-center py-20 text-[#CCBBA9]">Агентства не найдены</div>
          )}
      </div>
      <Footer />
      {showModal && (
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
              <Card className="w-full max-w-md p-6 relative animate-fadeIn">
                  <button onClick={() => setShowModal(false)} className="absolute top-4 right-4 text-[#AC8A69] hover:text-[#936142]"><X size={24}/></button>
                  <h3 className="text-2xl font-serif text-[#414942] mb-6">{editingAgency ? 'Редактировать' : 'Новое агентство'}</h3>
                  <div className="space-y-4">
                      <Input label="Название" placeholder="Wedding Stars" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                      <Input label="Email" placeholder="mail@example.com" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                      <Input type="text" label="Пароль" placeholder="Любой пароль" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
                      <Input label="Телефон" placeholder="+7..." value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                      <Button className="w-full mt-4" onClick={handleSave} disabled={isSaving}>{isSaving ? <Loader2 className="animate-spin mr-2"/> : <Save size={18} className="mr-2"/>}{editingAgency ? 'Сохранить' : 'Создать'}</Button>
                  </div>
              </Card>
          </div>
      )}
