...
// Отвечает за то, чтобы курсор не прыгал во время ввода
const TimingItem = ({ item, onUpdate, onRemove }) => {
    ...
    // Если данные обновились извне (например, после сортировки), синхронизируем
    useEffect(() => { setLocalTime(item.time); }, [item.time]);
    useEffect(() => { setLocalEvent(item.event); }, [item.event]);

    // Сохраняем в базу и СОРТИРУЕМ только когда убрали курсор (onBlur)
    const handleBlur = () => {
        // Если данные изменились, отправляем наверх
    ...
    return (
        <div className="relative pl-6 group print:pl-0 print:border-b print:pb-2 print:border-[#EBE5E0]">
            ...
            <div className="flex items-baseline gap-4">
                {/* ВРЕМЯ */}
                <input 
                    className="w-16 md:w-20 text-base md:text-lg font-bold text-[#936142] bg-transparent outline-none text-right font-mono print:text-left print:w-20 placeholder-red-300/50" 
                    value={localTime} 
                    placeholder="00:00"
                    onChange={(e) => setLocalTime(e.target.value)} 
                    onBlur={handleBlur} // <--- Срабатывает сортировка здесь
                />
                ...
                <button onClick={() => onRemove(item.id)} className="opacity-0 group-hover:opacity-100 text-[#CCBBA9] hover:text-red-400 p-1 print:hidden">
                    <X size={14}/>
                </button>
            </div>
        </div>
...
export const TimingView = ({ timing, updateProject, project }) => {
  ...
  const addTimingItem = () => { 
      ...
      // Добавляем в начало, сортировка не нужна (пустое время и так будет сверху)
    ...
    <div className="animate-fadeIn max-w-2xl mx-auto pb-24 md:pb-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 print:hidden">
        ...
        <div className="flex gap-2 w-full md:w-auto">
            <Button onClick={addTimingItem} className="flex items-center gap-2">
